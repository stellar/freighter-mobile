/**
 * Real pubnet history for one account, in the wire shape of the
 * freighter-backend-v2 account-history endpoint
 * (`GET /api/v1/accounts/{address}/transactions`).
 *
 * TEST-ONLY. Nothing under `src/` imports this — the app always calls the real
 * endpoint. It exists so the v2 mappers can be tested against genuine
 * on-chain data rather than only hand-written shapes; the hand-written
 * per-scenario transactions live in `./historyV2Scenarios.ts`. Do not import
 ...
 *     forward in time, while the array below is newest-first — the order the
 *     list is meant to render in. `useGetHistoryDataV2` does not sort, so that
 *     mismatch is still unresolved and has to be settled when pagination is
 *     wired up against the live endpoint.
 ...
 *
 * Map these with `publicKey: MOCK_SELF`. `decodeCounterparty` compares the
 * decoded destination against the wallet's own key, so passing any other key
 * makes that comparison never match and inbound payments name the destination
 * rather than the sender as counterparty.
 *
 ...
 */
import { V2AccountTransaction } from "config/historyV2Types";
