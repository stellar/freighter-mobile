...
describe("BalancesList", () => {
  ...
  // Home turns the CTA off once it owns the Add affordance via its floating
  // pill, so the two are never on screen together.
  it("should hide the funding CTA when showEmptyStateCta is false", () => {
    (useBalancesList as jest.Mock).mockReturnValue({
      balanceItems: [],
      scanResults: {},
      isLoading: false,
    ...
    // The empty state itself still explains the situation — only the action
    // moves out to the caller.
    expect(getByTestId("unfunded-account-empty-state")).toBeTruthy();
    expect(queryByTestId("fund-account-empty-state-button")).toBeNull();
  });

  it("should hide the Friendbot button too when showEmptyStateCta is false", () => {
    (useBalancesList as jest.Mock).mockReturnValue({
      balanceItems: [],
      scanResults: {},
      isLoading: false,
  ...
  it("should show the funding CTA by default on mainnet", () => {
    (useBalancesList as jest.Mock).mockReturnValue({
      balanceItems: [],
      scanResults: {},
      isLoading: false,
