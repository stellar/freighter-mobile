...
describe("mapAccountBalancesV2", () => {
  describe("native", () => {
    it("maps to the `XLM` key with server-provided available", () => {
      ...
      // v1 responses key native as "native" and fetchBalances renames it —
      // the v2 mapper emits the app convention directly, so no leftover key.
  ...
  describe("classic", () => {
    it("maps to the `code:issuer` key with issuer nested under token", () => {
      ...
      expect(entry.limit.toString()).toBe("1000");
  ...
  describe("SAC", () => {
    it("maps to a classic-shaped `code:issuer` entry (no double-scaling)", () => {
      ...
      // NOT treated as a Soroban balance (no contractId/decimals), so the
      // display path won't re-scale it by `decimals`.
      expect(entry.contractId).toBeUndefined();
