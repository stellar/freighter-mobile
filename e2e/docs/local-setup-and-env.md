...
already set.  
**Do not commit `.env`** or use a phrase that holds real funds.
...
- **Build**: Workflows set env from **secrets** and **vars**. Scripts
  ...
  defined.
- **E2E**: `E2E_TEST_RECOVERY_PHRASE` comes from **secrets**. It's passed to
  Maestro via `-e` and (on iOS) into the simulator clipboard for Import Wallet.
