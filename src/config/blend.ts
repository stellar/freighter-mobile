...
export const getBlendPoolId = (networkDetails: NetworkDetails) =>
  BLEND_FIXED_POOL_IDS[networkDetails.network];
 ...
 *
 * `calculateSpendableAmount` only subtracts the base reserve and the *inclusion*
 * fee (~0.00001 XLM), but a Blend `submit` is dominated by its resource fee —
 * measured at 546,395 stroops (~0.0546 XLM) against the live pool, roughly
 * 5,000x the inclusion fee. Without this buffer a Max deposit of XLM simulates
 * into an insufficient-balance error.
 *
