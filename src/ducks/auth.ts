...
const signIn = async ({ password }: SignInParams): Promise<void> => {
  ...
  // TEMPORARILY COMMENTED OUT: Private key consolidation - causes ~6-7s delay
  // This step ensures all accounts' private keys are in the temporary store
  // Performance data shows this takes ~6-7 seconds due to key manager decryption operations
  // The active account's private key is already in the temporary store (created above)
  // Other accounts' private keys can be loaded on-demand when switching accounts (lazy loading)
  // TODO: Re-enable after optimizing or moving to background task
