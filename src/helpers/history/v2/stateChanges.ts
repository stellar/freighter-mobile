...
import { V2StateChange } from "config/historyV2Types";
...
export const mapStateChangeCards = (
  ...
  // eslint-disable-next-line no-restricted-syntax -- ported verbatim from the extension's state-change grouping algorithm; the switch's fallthrough-free cases and the accumulator closures above read clearer than an array-method rewrite.
    ...
    switch (change.variant) {
      ...
      case "ThresholdChange":
        cards.push({
          kind: "thresholds",
          level:
            // eslint-disable-next-line no-nested-ternary -- ported verbatim from the extension; a nested ternary over the three threshold levels reads clearer than a lookup table here.
  ...
  // eslint-disable-next-line no-restricted-syntax -- ported verbatim; iterating a fixed 3-element verb tuple reads clearer than an array-method rewrite here.
  ...
  // eslint-disable-next-line no-restricted-syntax -- ported verbatim; see the signers loop's disable reason above.
  for (const verb of ["added", "updated", "removed"] as const) {
    const entries = dataEntriesByVerb.get(verb);
    if (entries) {
