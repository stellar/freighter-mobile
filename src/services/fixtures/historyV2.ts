...
/**
 * Addresses the synthetic state-change fixtures below reference as signers,
 * trustors, allowance spenders and account-creation/merge counterparties. Real,
 * well-formed pubnet keys (so `truncateAddress` and the Avatar render normally),
 * but the changes attributed to them are fabricated — see the block comment on
...
export const mockHistoryTransactions: V2AccountTransaction[] = [
  // ===========================================================================
  // SYNTHETIC state-change coverage (2026-08-05 .. 2026-08-16).
  //
  // Unlike everything below this block, these eleven transactions are NOT real
  // pubnet activity: the hashes are fabricated and the state changes are
  // hand-written. They exist because the captured/derived history further down
  // only ever emits BalanceChange, TrustlineAddedChange and
  // BlendEmissionsClaimChange — 3 of the 19 variants the mappers handle — so 9
  // of the detail sheet's 10 state-change card kinds had no way to render at
  // all during development.
  //
  // The three newest also cover multi-asset shapes the single-asset entries
  // cannot reach, matching the browser extension's own scenario fixtures
  // (@shared/api/fixtures/history-v2-scenarios.ts on feat/history-redesign):
  // a `classify` result of "multiple" (four tokens, with repeats), a trustline
  // group spanning three tokens and all three verbs, and a contract call that
  // charges a fee with no balance movement at all.
  //
  // Every `operation_xdr` here IS real, SDK-generated and round-trip decoded
  // (see the generator note below), so the advanced "Transaction details" view
  // decodes and renders them rather than showing "Failed to decode".
  //
  // Generated with @stellar/stellar-sdk `Operation.*` builders and verified by
  // decoding each one back through `Operation.fromXDRObject(xdr.Operation
  // .fromXDR(value, "base64"))` — the same call AdvancedDetails.decodeOperations
  // makes. To regenerate, rebuild with the same builders and re-run that
  // round-trip before pasting.
  //
  // These are dated NEWER than the real history so they sort to the top of the
  // first page; the mock's default `limit` was raised to fit them alongside the
  // real entries (see the note on mockFetchAccountHistoryV2). Delete this whole
  // block to get the real-only list back.
  // ---------------------------------------------------------------------------

  // 2026-08-16T09:48:31Z — INVOKE_HOST_FUNCTION router swap (synthetic)
  //   -> `amounts: "multiple"` on the list row + the Figma "Max width" detail.
  //
  // Four distinct tokens move, with repeats: `classify` in
  // helpers/history/v2/balances.ts only returns "sent"/"received" for a single
  // row and "swapped" for exactly one credit + one debit of different tokens,
  // so anything else lands here. Nothing in the real captured history below
  // reaches it — the swaps there are all clean 1-credit/1-debit pairs.
  //
  // The repeated yUSDC credits and CETES debits are deliberate (mirroring the
  // extension's own multi-asset fixture): they put two rows with the same
  // token on screen at once, which is what exercises row-key uniqueness in the
  // changes card rather than just the "more than two tokens" branch.
  //
  // The XLM debit is deliberately NOT equal to `fee_charged` — balances.ts
  // treats a lone XLM debit matching fee_charged as the fee row and drops it,
  // which would quietly change the classification.
  {
    hash: "1a2b3c4d5e6f708192a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e7f809",
    fee_charged: "412337",
    result_code: "TransactionResultCodeTxSuccess",
    ledger_number: 63967061,
    ledger_created_at: "2026-08-16T09:48:31Z",
    is_fee_bump: false,
    ingested_at: "2026-08-16T09:48:38Z",
    operations: [
      {
        id: "274800000000000001",
        operation_type: "INVOKE_HOST_FUNCTION",
        operation_xdr:
          "",
  ...
  // 2026-08-15T11:22:07Z — CHANGE_TRUST x3 (synthetic)
  //   -> one trustlines group spanning THREE tokens and all three verbs, the
  //   Figma "Multiple trustline change" frame. The single-verb pairs further
  //   down cover each verb in isolation; this covers them interleaved, which is
  //   where per-verb card grouping actually has to hold.
  {
    hash: "2b3c4d5e6f708192a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e7f80912",
    fee_charged: "300",
    result_code: "TransactionResultCodeTxSuccess",
    ledger_number: 63949781,
    ledger_created_at: "2026-08-15T11:22:07Z",
    is_fee_bump: false,
    ingested_at: "2026-08-15T11:22:14Z",
    operations: [
      {
        id: "274700000000000001",
        operation_type: "CHANGE_TRUST",
        operation_xdr:
          "AAAAAAAAAAYAAAACeVVTREMAAAAAAAAAAAAAAM062kzjDS5P+fW5F9nCOTSPRGUSC5NLY3A9tdoTi3LZf/////////8=",
        result_code: "op_success",
        successful: true,
        ledger_number: 63949781,
        ledger_created_at: "2026-08-15T11:22:07Z",
      ...
      {
        id: "274700000000000002",
        operation_type: "CHANGE_TRUST",
        operation_xdr:
          "AAAAAAAAAAYAAAABQkxORAAAAADSQ8wk9k9LysVHuaGIy6OIJZ3l2qVzvvOU0/JtJfMh0gAAABdIdugA",
        result_code: "op_success",
        successful: true,
        ledger_number: 63949781,
        ledger_created_at: "2026-08-15T11:22:07Z",
      ...
      {
        id: "274700000000000003",
        operation_type: "CHANGE_TRUST",
        operation_xdr:
          "AAAAAAAAAAYAAAACQ0VURVMAAAAAAAAAAAAAAKOKGH1tQRNeffXJlHRIKKBJ4OB8ITO7TtohmZxn4sE2AAAAAAAAAAA=",
        result_code: "op_success",
        successful: true,
        ledger_number: 63949781,
        ledger_created_at: "2026-08-15T11:22:07Z",
    ...
    state_changes: [
      {
        ...
        ingested_at: "2026-08-15T11:22:14Z",
        token_id: MOCK_YUSDC_SAC,
        // int64 max in decimal XLM units — the "no limit" trustline
  ...
  // 2026-08-14T17:05:53Z — INVOKE_HOST_FUNCTION with NO state changes
  //   (synthetic) -> the Figma "No token change balance" frame: a contract call
  //   that charges a fee without moving a balance. `classify` returns
  //   { type: "none" }, so the row has no amount and the sheet renders no
  //   changes card at all. The real history has organic cases of this
  //   (CREATE_CLAIMABLE_BALANCE as claimant), but none via a contract call.
  {
    hash: "3c4d5e6f708192a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e7f8091234",
    fee_charged: "398211",
    result_code: "TransactionResultCodeTxSuccess",
    ledger_number: 63932921,
    ledger_created_at: "2026-08-14T17:05:53Z",
    is_fee_bump: false,
    ingested_at: "2026-08-14T17:06:00Z",
    operations: [
      {
        id: "274650000000000001",
        operation_type: "INVOKE_HOST_FUNCTION",
        operation_xdr:
          "AAAAAAAAABgAAAAAAAAAAdxbfO1S6ZisuZT4S367BXiUdQdk/Bfe8saQQueNJ2dqAAAACHRyYW5zZmVyAAAAAwAAABIAAAAAAAAAAIIxdBV+SOk7aAAlDMqxSw990ELJ+u/Luyvz7flw7E5LAAAAEgAAAAAAAAAAYvwdC9CRsrYcDdZWNGsqaNfTR8bywsjubQRHAlb8BfcAAAAKAAAAAAAAAAAAAAAAGBSNAAAAAAA=",
        result_code: "op_success",
        successful: true,
        ledger_number: 63932921,
        ledger_created_at: "2026-08-14T17:05:53Z",
  ...
  // 2026-08-12T10:14:02Z — ACCOUNT_MERGE (synthetic) -> accountMerged card
  {
    hash: "f1a2b3c4d5e6f708192a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e7f80",
    fee_charged: "100",
    result_code: "TransactionResultCodeTxSuccess",
    ledger_number: 63932501,
    ledger_created_at: "2026-08-12T10:14:02Z",
    is_fee_bump: false,
    ingested_at: "2026-08-12T10:14:09Z",
    operations: [
      {
        id: "274600000000000001",
        operation_type: "ACCOUNT_MERGE",
        operation_xdr:
          "AAAAAAAAAAgAAAAAo4oYfW1BE1599cmUdEgooEng4HwhM7tO2iGZnGfiwTY=",
        result_code: "op_success",
        successful: true,
        ledger_number: 63932501,
        ledger_created_at: "2026-08-12T10:14:02Z",
    ...
    state_changes: [
      {
        ...
        ledger_created_at: "2026-08-12T10:14:02Z",
        ingested_at: "2026-08-12T10:14:09Z",
        destination_address: MOCK_MERGE_DESTINATION,
      },
      // A merge sweeps the remaining XLM out, so the card sits beside a debit.
      {
        variant: "BalanceChange",
        type: "BALANCE",
        reason: "DEBIT",
  ...
  // 2026-08-11T09:02:44Z — CREATE_ACCOUNT (synthetic) -> accountCreated card
  {
    hash: "e2b3c4d5e6f708192a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e7f8091",
    fee_charged: "100",
    result_code: "TransactionResultCodeTxSuccess",
    ledger_number: 63915221,
    ledger_created_at: "2026-08-11T09:02:44Z",
    is_fee_bump: false,
    ingested_at: "2026-08-11T09:02:51Z",
    operations: [
      {
        id: "274500000000000001",
        operation_type: "CREATE_ACCOUNT",
        // destination is MOCK_SELF: the mapper reads AccountCreatedChange as
        // "this account was created", so the created account is the wallet's
        // own key and creator_address below is the funder.
        operation_xdr:
          "AAAAAAAAAAAAAAAAgjF0FX5I6TtoACUMyrFLD33QQsn678u7K/Pt+XDsTksAAAAAAvrwgA==",
        result_code: "op_success",
        successful: true,
        ledger_number: 63915221,
        ledger_created_at: "2026-08-11T09:02:44Z",
    ...
    state_changes: [
      {
        ...
        ledger_created_at: "2026-08-11T09:02:44Z",
        ingested_at: "2026-08-11T09:02:51Z",
        // The funder — someone else. `buildStateChangeCards` maps this change
        // to { address: publicKey, funder: creator_address }, i.e. "this
        // account was created and this address paid for it", so MOCK_SELF here
        // would render the account as its own funder.
        creator_address: MOCK_SIGNER_A,
      },
      {
        // CREDIT, not DEBIT: a newly created account *receives* its starting
        // balance. The funder pays the fee, so there is no fee debit either.
        variant: "BalanceChange",
        type: "BALANCE",
        reason: "CREDIT",
        ledger_number: 63915221,
  ...
  // 2026-08-10T16:41:19Z — INVOKE_HOST_FUNCTION approve (synthetic)
  //   -> allowance card (the kind with no Figma yet)
  {
    hash: "d3c4d5e6f708192a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e7f809123",
    fee_charged: "412337",
    result_code: "TransactionResultCodeTxSuccess",
    ledger_number: 63897941,
    ledger_created_at: "2026-08-10T16:41:19Z",
    is_fee_bump: false,
    ingested_at: "2026-08-10T16:41:26Z",
    operations: [
      {
        id: "274400000000000001",
        operation_type: "INVOKE_HOST_FUNCTION",
        operation_xdr:
          "AAAAAAAAABgAAAAAAAAAAdxbfO1S6ZisuZT4S367BXiUdQdk/Bfe8saQQueNJ2dqAAAAB2FwcHJvdmUAAAAABAAAABIAAAAAAAAAAIIxdBV+SOk7aAAlDMqxSw990ELJ+u/Luyvz7flw7E5LAAAAEgAAAAAAAAAAYvwdC9CRsrYcDdZWNGsqaNfTR8bywsjubQRHAlb8BfcAAAAKAAAAAAAAAAAAAAAAlQL5AAAAAAMDynWAAAAAAA==",
        result_code: "op_success",
        successful: true,
        ledger_number: 63897941,
        ledger_created_at: "2026-08-10T16:41:19Z",
    ...
    state_changes: [
      {
        ...
        ingested_at: "2026-08-10T16:41:26Z",
        token_id: MOCK_YUSDC_SAC,
        spender: MOCK_SIGNER_A,
        amount: "2500000000",
        expiration_ledger: 63600000,
      },
    ],
  },

  // 2026-08-09T13:27:55Z — SET_TRUST_LINE_FLAGS x3 (synthetic)
  //   -> balanceAuthorizations cards, both the SET and the CLEAR wording, with
  //   the authorized side spanning two assets: `buildStateChangeCards`
  //   accumulates authorized/unauthorized tokens into one card each, so a
  //   single-token fixture never shows whether the multi-token list renders.
  {
    hash: "c4d5e6f708192a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e7f80912a3b",
    fee_charged: "200",
    result_code: "TransactionResultCodeTxSuccess",
    ledger_number: 63880661,
    ledger_created_at: "2026-08-09T13:27:55Z",
    is_fee_bump: false,
    ingested_at: "2026-08-09T13:28:02Z",
    operations: [
      {
        id: "274300000000000001",
        operation_type: "SET_TRUST_LINE_FLAGS",
        operation_xdr:
          "AAAAAAAAABUAAAAAYvwdC9CRsrYcDdZWNGsqaNfTR8bywsjubQRHAlb8BfcAAAACeVVTREMAAAAAAAAAAAAAAM062kzjDS5P+fW5F9nCOTSPRGUSC5NLY3A9tdoTi3LZAAAAAAAAAAE=",
        result_code: "op_success",
        successful: true,
        ledger_number: 63880661,
        ledger_created_at: "2026-08-09T13:27:55Z",
      ...
      {
        id: "274300000000000002",
        operation_type: "SET_TRUST_LINE_FLAGS",
        operation_xdr:
          "AAAAAAAAABUAAAAAYvwdC9CRsrYcDdZWNGsqaNfTR8bywsjubQRHAlb8BfcAAAABQkxORAAAAADSQ8wk9k9LysVHuaGIy6OIJZ3l2qVzvvOU0/JtJfMh0gAAAAAAAAAB",
        result_code: "op_success",
        successful: true,
        ledger_number: 63880661,
        ledger_created_at: "2026-08-09T13:27:55Z",
      ...
      {
        id: "274300000000000003",
        operation_type: "SET_TRUST_LINE_FLAGS",
        operation_xdr:
          "AAAAAAAAABUAAAAAYvwdC9CRsrYcDdZWNGsqaNfTR8bywsjubQRHAlb8BfcAAAACQ0VURVMAAAAAAAAAAAAAAKOKGH1tQRNeffXJlHRIKKBJ4OB8ITO7TtohmZxn4sE2AAAAAQAAAAA=",
        result_code: "op_success",
        successful: true,
        ledger_number: 63880661,
        ledger_created_at: "2026-08-09T13:27:55Z",
  ...
  // 2026-08-08T08:55:31Z — CHANGE_TRUST x2 (synthetic)
  //   -> trustlines cards for the two verbs the real history never emits
  {
    hash: "b5e6f708192a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c",
    fee_charged: "200",
    result_code: "TransactionResultCodeTxSuccess",
    ledger_number: 63863381,
    ledger_created_at: "2026-08-08T08:55:31Z",
    is_fee_bump: false,
    ingested_at: "2026-08-08T08:55:38Z",
    operations: [
      {
        id: "274200000000000001",
        operation_type: "CHANGE_TRUST",
        operation_xdr:
          "AAAAAAAAAAYAAAACeVVTREMAAAAAAAAAAAAAAM062kzjDS5P+fW5F9nCOTSPRGUSC5NLY3A9tdoTi3LZAAAAdGpSiAA=",
        result_code: "op_success",
        successful: true,
        ledger_number: 63863381,
        ledger_created_at: "2026-08-08T08:55:31Z",
    ...
    state_changes: [
      {
        ...
        ingested_at: "2026-08-08T08:55:38Z",
        token_id: MOCK_YUSDC_SAC,
        old_limit: "100000000000",
        new_limit: "500000000000",
      },
      {
        variant: "TrustlineRemovedChange",
        type: "TRUSTLINE",
  ...
  // 2026-08-07T15:12:08Z — MANAGE_DATA x3 (synthetic)
  //   -> dataEntry cards, one per verb; each key row opens the data-entry view.
  //   Values are base64 of "dark" / "light" / "freighter-dev" / "fdev" /
  //   "session-abc123" — printable UTF-8, so the decoded view shows text
  //   rather than falling back to the raw base64.
  {
    hash: "a6f708192a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d",
    fee_charged: "300",
    result_code: "TransactionResultCodeTxSuccess",
    ledger_number: 63846101,
    ledger_created_at: "2026-08-07T15:12:08Z",
    is_fee_bump: false,
    ingested_at: "2026-08-07T15:12:15Z",
    operations: [
      {
        id: "274100000000000001",
        operation_type: "MANAGE_DATA",
        operation_xdr: "AAAAAAAAAAoAAAAMY29uZmlnLnRoZW1lAAAAAQAAAARkYXJr",
        result_code: "op_success",
        successful: true,
        ledger_number: 63846101,
        ledger_created_at: "2026-08-07T15:12:08Z",
      ...
      {
        id: "274100000000000002",
        operation_type: "MANAGE_DATA",
        operation_xdr:
          "AAAAAAAAAAoAAAAQcHJvZmlsZS5uaWNrbmFtZQAAAAEAAAANZnJlaWdodGVyLWRldgAAAA==",
        result_code: "op_success",
        successful: true,
        ledger_number: 63846101,
        ledger_created_at: "2026-08-07T15:12:08Z",
      ...
      {
        id: "274100000000000003",
        operation_type: "MANAGE_DATA",
        operation_xdr: "AAAAAAAAAAoAAAAObGVnYWN5LnNlc3Npb24AAAAAAAA=",
        result_code: "op_success",
        successful: true,
        ledger_number: 63846101,
        ledger_created_at: "2026-08-07T15:12:08Z",
    ...
    state_changes: [
      ...
      },
      {
        ...
        ingested_at: "2026-08-07T15:12:15Z",
        name: "legacy.session",
        old_value: "c2Vzc2lvbi1hYmMxMjM=",
      },
    ],
  },

  // 2026-08-06T11:33:47Z — SET_OPTIONS x2 (synthetic)
  //   -> homeDomain and flags cards (flags exercise both set and cleared)
  {
    hash: "97708192a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6",
    fee_charged: "200",
    result_code: "TransactionResultCodeTxSuccess",
    ledger_number: 63828821,
    ledger_created_at: "2026-08-06T11:33:47Z",
    is_fee_bump: false,
    ingested_at: "2026-08-06T11:33:54Z",
    operations: [
      {
        id: "274000000000000001",
        operation_type: "SET_OPTIONS",
        operation_xdr:
          "AAAAAAAAAAUAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAQAAAA1mcmVpZ2h0ZXIuYXBwAAAAAAAAAA==",
        result_code: "op_success",
        successful: true,
        ledger_number: 63828821,
        ledger_created_at: "2026-08-06T11:33:47Z",
      ...
      {
        id: "274000000000000002",
        operation_type: "SET_OPTIONS",
        operation_xdr:
          "AAAAAAAAAAUAAAAAAAAAAQAAAAIAAAABAAAAAQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA==",
        result_code: "op_success",
        successful: true,
        ledger_number: 63828821,
        ledger_created_at: "2026-08-06T11:33:47Z",
  ...
  // 2026-08-05T14:20:12Z — SET_OPTIONS x3 (synthetic)
  //   -> signers card (all three verbs share one card) + thresholds card.
  //   The removed signer carries old_weight; the updated one carries both
  //   weights, so the "old -> new" and "Added/Removed N" wordings both render.
  {
    hash: "8808192a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e7f80912a3b4c5d6e",
    fee_charged: "300",
    result_code: "TransactionResultCodeTxSuccess",
    ledger_number: 63811541,
    ledger_created_at: "2026-08-05T14:20:12Z",
    is_fee_bump: false,
    ingested_at: "2026-08-05T14:20:19Z",
    operations: [
      {
        id: "273900000000000001",
        operation_type: "SET_OPTIONS",
        operation_xdr:
          "AAAAAAAAAAUAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAEAAAAAYvwdC9CRsrYcDdZWNGsqaNfTR8bywsjubQRHAlb8BfcAAAAC",
        result_code: "op_success",
        successful: true,
        ledger_number: 63811541,
        ledger_created_at: "2026-08-05T14:20:12Z",
      ...
      {
        id: "273900000000000002",
        operation_type: "SET_OPTIONS",
        operation_xdr:
          "AAAAAAAAAAUAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAEAAAAAzTraTOMNLk/59bkX2cI5NI9EZRILk0tjcD212hOLctkAAAAA",
        result_code: "op_success",
        successful: true,
        ledger_number: 63811541,
        ledger_created_at: "2026-08-05T14:20:12Z",
      ...
      {
        id: "273900000000000003",
        operation_type: "SET_OPTIONS",
        operation_xdr:
          "AAAAAAAAAAUAAAAAAAAAAAAAAAAAAAAAAAAAAQAAAAEAAAABAAAAAgAAAAEAAAADAAAAAAAAAAA=",
        result_code: "op_success",
        successful: true,
        ledger_number: 63811541,
        ledger_created_at: "2026-08-05T14:20:12Z",
  ...
  // ===========================================================================
  // Real captured/derived pubnet history begins here.
  // ===========================================================================
...
export const mockFetchAccountHistoryV2 = async ({
  // 24, not the extension's 10: the synthetic state-change block at the top of
  // `mockHistoryTransactions` is 11 transactions, so a 10-item page would show
  // none of the real captured history behind it — and pagination is not wired
  // up yet (`ducks/history.ts` calls getAccountHistoryV2 with no limit and
  // never requests a second page), so anything past the first page is
  // unreachable in the app rather than merely one scroll away. Once infinite
  // scroll lands, this should go back to the real endpoint's page size.
  limit = 24,
  cursor,
}: {
  address?: string;
  limit?: number;
  cursor?: string;
  // Copied verbatim from the browser extension; kept async so the signature
  // matches the real endpoint call it stands in for. Do not add an await to
